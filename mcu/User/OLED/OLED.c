...
void OLED_ShowCharArea(uint8_t RangeX,uint8_t RangeY,uint8_t RangeWidth,uint8_t RangeHeight, int16_t X, int16_t Y, char Char, uint8_t FontSize)
{
	...
	else if(FontSize == OLED_7X12)	//字体为宽7像素，高12像素
	{
		/*将ASCII字模库OLED_F6x8的指定数据以6*8的图像格式显示*/
...
/**
  * 函    数：OLED显示混合字符串（汉字与ASCII） (GB2312)
...
/**
  * 函    数：在指定区域范围内OLED显示混合字符串（汉字与ASCII）(GB2312)
  * 参    数：RangeX 指定字符可以显示范围的左上角的横坐标，范围：负值~127
  * 参    数：RangeY 指定字符可以显示范围的左上角的纵坐标，范围：负值~63
  * 参    数：RangeWidth 指定范围宽度
  * 参    数：RangeHeight 指定范围高度
...
{
    FontSize = 16;//目前仅支持16*16
    uint8_t font_buffer[32];
	if(Find_Chinese(Chinese,font_buffer) == FR_OK)
    {
        uint8_t i,j;
        uint16_t temp,x0,y0;
        uint16_t size=(FontSize/8+((FontSize%8)?1:0))*FontSize;  //得到字体一个字符对应点阵集所占的字节数,即32
        x+=1,y+=1,x0=x,y0=y;
        for(i=0;i<size;i++)
        {
            temp=font_buffer[i];
